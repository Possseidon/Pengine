...
type
  ...
  IDoublyLinked = interface
    function GetLink(AIndex: Integer): TDoublyLink;
    function AddLink(ALink: TDoublyLink): Integer;
    procedure DelLink(AIndex: Integer);
  end;
  ...
  TDoublyLinked = class (TInterfacedObject, IDoublyLinked)
  private
    FLinks: TObjectArray<TDoublyLink>;
  ...
  TObjectList<T: IDoublyLinked> = class
  private
    type
      TCompareFunction = function (A, B: T): Boolean;
  ...
  private
    ...
    procedure Sort(ACompare: TCompareFunction; ALeft, ARight: TDoublyLink; ACount: Integer); overload;

    function GetFirst: T;
    function GetLast: T;

    procedure DelLink(ALink: TDoublyLink);
...
constructor TDoublyLink.Create(AData: TObject);
begin
  Data := AData;
end;
...
function TDoublyLinked.GetLink(AIndex: Integer): TDoublyLink;
begin
  Result := FLinks[AIndex];
end;

function TDoublyLinked.AddLink(ALink: TDoublyLink): Integer;
begin
  Result := FLinks.Count;
  FLinks.Add(ALink);
end;

procedure TDoublyLinked.DelLink(AIndex: Integer);
begin
  FLinks.DelIndex(AIndex);
end;

constructor TDoublyLinked.Create;
begin
  FLinks := TObjectArray<TDoublyLink>.Create;
end;

destructor TDoublyLinked.Destroy;
begin
  FLinks.Free;
  inherited Destroy;
end;
