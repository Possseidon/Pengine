...
type
  ...
  public
    ...
    Size: Int64;
    Chunk: TMemoryStream; // Chunk Stream
...
function TVerySimpleLua.DoStream(Stream: TStream; Size: Int64=0; ChunkName: String=''): Integer;
var
  ChunkStream: TLuaChunkStream;
  Marshall: TMarshaller;
begin
...
class procedure TVerySimpleLua.PushFunction(L: lua_State; Data, Code: Pointer);
begin
...
class function TVerySimpleLua.ValidProperty(LProperty: TRttiProperty): Boolean;
var
  Attribute: TCustomAttribute;
begin
  Result := False;

  // Only published property allowed
...
class procedure TVerySimpleLua.RegisterTableFunction(L: lua_State; Data, Code: Pointer; FuncName: String);
var
  Marshall: TMarshaller;
begin
  PushFunction(L, Data, Code);
...
class procedure TVerySimpleLua.RegisterTableFunction(L: lua_State; AObject: TObject; Func: String);
begin
  RegisterTableFunction(L, Pointer(AObject), AObject.MethodAddress(Func), Func);
end;

class procedure TVerySimpleLua.RegisterTableFunctions(L: lua_State; AObject: TObject);
var
  LContext: TRttiContext;
  LType: TRttiType;
  LMethod: TRttiMethod;
...
function TVerySimpleLua.Report(L: Lua_State; Status: Integer): Integer;
var
  Msg: String;
begin
  if (Status <> LUA_OK)  then
