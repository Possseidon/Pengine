...
type
  ...
  public
    ...
    function GUIToClientX(ABounds: TGBounds1): TGBounds1; overload;
    function GUIToClientX(APoint: Single): Single; overload;

    function GUIToClientY(ABounds: TGBounds1): TGBounds1; overload;
    function GUIToClientY(APoint: Single): Single; overload;
  ...
  TButtonState = (bsDefault, bsHover, bsPressed, bsDisabled);
...
begin
  ...
  Result.C2.X := Result.C1.X + Height * ParentControl.ClientToGUIY(0.5);
end;
...
begin
  ...
  FTextDisplay.Pos := ParentControl.ClientToGUI(GetBounds.LeftMid + UVecX * Height * (1 - TextScale));
  // for Offset testing
  // FTextDisplay.Pos := ParentControl.ClientToGUI(GetBounds.LeftMid + UVecX * Height * ((1 - TextScale) - Offset));
...
begin
  ...
  begin
    ...
    CursorPos := FTextDisplay.IndexFromOffset((FGLForm.Input.MousePos.X - L) / FTextDisplay.Height, haCenter) +
      FOffsetSkippedChars;
  end;
  ...
  begin
    ...
    begin
      if FGLForm.Input.KeyDown(VK_CONTROL) then
        Text := Copy(Text, 0, CursorPos) + Copy(Text, StrRFind(Text, CursorPos + CountSpaces(Text, CursorPos)) + 2,
          Length(Text) - 1)
      else
...
begin
  ...
  FTextDisplay.Pos := ParentControl.ClientToGUI(GetBounds.Middle);
  FTextDisplay.Depth := Depth;
  FTextDisplay.AddToVAO(FFontVAO);
end;
...
begin
  ...
  begin
    ...
    FTextDisplays[I].Pos := ParentControl.ClientToGUI
      (TGVector2.Create(P.X, P.Y - I * (LineHeight + LineSpacing * LineHeight)));
    FTextDisplays[I].Color := Color;
    FTextDisplays[I].XOrigin := XOrigin;
    FTextDisplays[I].Italic := Italic;
    FTextDisplays[I].Height := ParentControl.ClientToGUIY(TGBounds1.Create(0, LineHeight)).Length;
...
begin
  Result := ParentControl.ClientToGUI(Pos);
end;
...
begin
  ...
  case XOrigin of
    haLeft:
      Result := Result - UVecX * ParentControl.Width / 2;
    haRight:
      Result := Result + UVecX * ParentControl.Width / 2;
  end;
  ...
  case YOrigin of
    vaTop:
      Result := Result + UVecY * ParentControl.Height / 2;
    vaBottom:
      Result := Result - UVecY * ParentControl.Height / 2;
  end;
...
begin
  ...
  case XOrigin of
    haLeft:
      Result := Result - ParentControl.Width / 2;
    haRight:
      Result := Result + ParentControl.Width / 2;
  end;
  ...
  if FParentControl <> nil then
    Result := FParentControl.ClientToGUIX(B.Horizontal[Result]);
end;
...
begin
  ...
  case YOrigin of
    vaTop:
      Result := Result + ParentControl.Height / 2;
    vaBottom:
      Result := Result - ParentControl.Height / 2;
  end;
  ...
  if FParentControl <> nil then
    Result := FParentControl.ClientToGUIY(B.Vertical[Result]);
end;
...
function TGLControl.GUIToClientX(APoint: Single): Single;
var
  B: TGBounds1;
begin
  Result := APoint;
  if FParentControl <> nil then
    Result := FParentControl.GUIToClientX(Result);

  B := GetBounds.Horizontal;
...
function TGLControl.GUIToClientX(ABounds: TGBounds1): TGBounds1;
begin
  Result.C1 := GUIToClientX(ABounds.C1);
  Result.C2 := GUIToClientX(ABounds.C2);
end;

function TGLControl.GUIToClientY(APoint: Single): Single;
var
  B: TGBounds1;
begin
  Result := APoint;
  if FParentControl <> nil then
    Result := FParentControl.GUIToClientY(Result);

  B := GetBounds.Vertical;
...
function TGLControl.GUIToClientY(ABounds: TGBounds1): TGBounds1;
begin
  Result.C1 := GUIToClientY(ABounds.C1);
  Result.C2 := GUIToClientY(ABounds.C2);
end;
...
begin
  if ParentControl = nil then
    ...
    Result := ParentControl.ClientToGUI(GetBounds);

  case XOrigin of
    haLeft:
      Result.Translate(-UVecX * ParentControl.Width / 2);
    haRight:
      Result.Translate(+UVecX * ParentControl.Width / 2);
  end;
  ...
  case YOrigin of
    vaTop:
      Result.Translate(+UVecY * ParentControl.Height / 2);
    vaBottom:
      Result.Translate(-UVecY * ParentControl.Height / 2);
  end;
end;
