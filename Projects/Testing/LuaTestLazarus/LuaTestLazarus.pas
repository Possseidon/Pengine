program LuaTestLazarus;

{$R *.res}

uses
  SysUtils, LuaHeader, Classes, Math;

function L_LoadString(L: TLuaState): Integer;
begin
...
function L_ToString(L: TLuaState): Integer;
begin
  L.CheckAny(1);
  L.CheckEnd(2);
  L.ToString_X;
...
function L_Print(L: TLuaState): Integer;
var
  Params, I: Integer;
begin
  Params := L.Top;
...
function L_Error(L: TLuaState): Integer;
var
  Level: Integer;
begin
  if L.IsNone(1) then
...
function L_Next(L: TLuaState): Integer;
begin
...
function L_Type(L: TLuaState): Integer;
var
  T: TLuaType;
begin
  T := L.CheckAny(1);
...
function L_PCall(L: TLuaState): Integer;
begin
...
function L_GetMetatable(L: TLuaState): Integer;
begin
  L.CheckType(1, ltTable);
  L.CheckEnd(2);
...
function L_PrintArray(L: TLuaState): Integer;
var
  I: Integer;
  Digits: Integer;
  S: AnsiString;
begin
  ...
  begin
    if L.GetI(I, 1) = ltNil then
      Break;
    L.PushValue(2);
    S := Format(Format('[%%%dd] = %%s', [Digits]), [I, L.ToString(3)]);
...
function L_SetMetatable(L: TLuaState): Integer;
begin
  L.CheckType(1, ltTable);
