...
{
  return color * fcolor * calcDiffuseFactor(-lightvec, normal);
}

vec3 calcSpecular(vec3 color, vec3 reflected)
{
  return color * fcolor * pow(calcDiffuseFactor(reflected, normalize(fcam - fpos)), 10);
}
...
void main()
{
  ...
  outcolor = vec4(ambient * fcolor, 1);
