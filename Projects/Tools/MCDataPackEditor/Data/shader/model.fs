...
{  
  return color * fcolor * texture(diffusemap, ctexcoord).rgb * calcDiffuseFactor(-lightvec, normal); 
}

vec3 calcSpecular(vec3 color, vec3 reflected)
{
  return color * 0.2 * pow(calcDiffuseFactor(reflected, normalize(fcam - fpos)), 10);
}
